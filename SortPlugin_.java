  ...
  implements PlugIn
{
  ...
  static {
    try { FileHandler handler = new FileHandler("SortPlugin.log", 100000, 2, true);
    ...
    throws IOException
  {
    ...
    try {
      ...
      System.setProperty("java.class.path", System.getProperty("java.class.path") + u.toExternalForm());
    } catch (Throwable t) {
  ...
  private void addExternalLibraries() throws IOException
  {
    ...
    for (int stop = files.length; start < stop; start++) {
      ...
      logger.log(Level.FINEST, "added library : " + files[start].toURL() + " to classpath");
    }
  }
  ...
  public void run(String arg) {
    try {
      ...
      getClass(); Class entryPoint = Class.forName(this.properties.getString("plugin.entry.point.class"));
      Object startingPoint = entryPoint.newInstance();
      Method startingMethod = entryPoint.getDeclaredMethod(this.properties.getString("plugin.entry.point.method"), null);
