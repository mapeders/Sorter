	...
	public void sort() {
		...
		while (it.hasNext()) {
			...
			try {
				...
				for (StringTokenizer tokens = new StringTokenizer(a, System.getProperty("line.separator")); tokens.hasMoreTokens();) {
				...
				File patientDir = new File(this.targetDir + File.separator + patientName.getValue());
				if (!patientDir.exists()) {
					logger.debug(patientDir.getAbsolutePath());
					patientDir.mkdir();
				}
				String newFileName = File.separator + getFilePrefix(metaData, this.metaTags) + ".dcm";
				File targetFile = new File(patientDir + File.separator + newFileName);
				logger.debug("copying target file " + targetFile.getAbsolutePath());
